...
> **Adeguamento 24–25 ago 2026:** aggiunta la modalità di licenza a **utilizzo
> singolo** (`subscription_type = 'single_use'`) e una nuova sezione 8 con le
> Edge Function reali del sistema di licenze, entrambe verificate direttamente
> sul progetto Supabase esterno condiviso (schema + codice sorgente). Le
> modifiche sono segnalate con blockquote `>`. Per il quadro completo del
> sistema di licensing vedi `sistema-accesso-licensing-schema.md`.
...
- `licenses`: `id`, `license_key`, `app_code`, `is_active`, `expires_at`, `activated_at`,
  `subscription_type`, `pdf_exports_remaining`.
  Una licenza = N posti (seat).
  > **Novità:** `subscription_type` può valere `'single_use'` (oltre a `'trial'`,
  > `'annual'`, ecc.). Per queste licenze, tipicamente `seats = 1` e
  > `pdf_exports_remaining` funge da plafond export (di norma `1`), coerente con
  > `max_pdf_exports` in `product_catalog`.
...
3. **Scadenza** — se `expires_at` valorizzata e `<= now` → `E-103`.
   > Per le licenze `single_use`, `expires_at` è tipicamente `NULL` fino al
   > completamento di questo stesso passaggio (vedi punto 9 aggiornato), quindi
   > questo controllo in pratica non blocca mai la *prima* attivazione.
...
9. **Prima attivazione della licenza** — se `licenses.activated_at` è NULL,
   ...
   > **Novità — licenze `single_use` (confermato dal codice reale delle Edge
   > Function, sezione 8):** alla creazione, `licenses.expires_at` resta
   > **`NULL`** apposta — le **48 ore** di validità partono dal **primo accesso
   > reale**, non dall'acquisto né dalla creazione della riga. Il commento nel
   > codice di `license-generator-programma` attribuisce questo calcolo a
   > "`verifyAndActivateLicense` lato app", cioè proprio a questo passaggio 9:
   > nello stesso update guardato da `.is('activated_at', null)` va quindi
   > scritto anche `expires_at = now() + 48h` quando `subscription_type ===
   > 'single_use'`. **Da verificare in `license.functions.ts`** che le 48h
   > siano effettivamente implementate lì (il valore non è ricavato da
   > `product_catalog.validity_days`, che per il single-use ha un significato
   > diverso — vedi sezione 8).
10. Successo → `{ ok:true, reactivated, licenseId, pukId, userId }`.

Qualsiasi eccezione → catch globale → `E-500`.

> **Nota sul resto del ciclo di vita `single_use`** (fuori da questo funnel di
> attivazione): alla generazione del PDF/report finale del prodotto,
> `licenses.is_active` viene posto a `false`, chiudendo definitivamente
> l'accesso indipendentemente da `expires_at`. Questo passaggio vive nella
> logica applicativa del prodotto (es. Edge Function di generazione report),
> non nel funnel `/attivazione` descritto qui.
...
6. Nel DB esterno: inserisci le `licenses` con il nuovo `app_code`, genera i
   ...
   > Se il prodotto offre anche l'opzione **single-use**, valorizza
   > `subscription_type = 'single_use'` e lascia `expires_at = NULL` alla
   > creazione: verrà scritto automaticamente al primo claim (punto 9 della
   > sezione 2). Assicurati che `product_catalog` abbia una riga con
   > `validity_days` e `max_pdf_exports` coerenti per quell'`app_code`.
...
10. Test end-to-end: email nuova → OTP → licenza+PUK → condizioni → app;
    ...
    > Per il caso single-use, verifica anche: `expires_at` scritto al primo
    > claim, e che dopo la generazione del PDF/report `is_active` passi a
    > `false` impedendo un secondo accesso.

---

## 8. Edge Functions del sistema di licenze

Queste function vivono nel progetto Supabase **esterno condiviso**
(`ruopxyprezzxoirfrjrm`) e sono a monte del funnel `/auth → /attivazione →
/condizioni`: generano/rinnovano le righe in `licenses` e `puk_codes` che il
funnel poi consuma. Elenco verificato via introspezione diretta del progetto
il 25 ago 2026 (tutte `ACTIVE`, `verify_jwt: true`).

| Function | Ruolo | Family |
|---|---|---|
| `license-generator-programma` | Genera licenza + PUK da un acquisto (webhook Paddle, oggi in modalità test) per i **programmi/SaaS** (es. `002MnFAT`) | programma |
| `license-generator-corso` | Come sopra ma per i **corsi** (es. GDPR): richiede `course_id`, non supporta `single_use` | corso |
| `license-renew-programma` | Rinnova una licenza esistente (`monthly`/`annual`) estendendone `expires_at` | programma |
| `test-license-generator` | Versione precedente di `license-renew-programma`, senza il blocco sui `single_use` — probabile residuo da dismettere | legacy |
| `verify-license` | Verifica email+licenza+PUK e li attiva; versione generica più vecchia, **non aggiornata** con la logica `single_use`/`pdf_exports_remaining` | legacy/generica |
| `paddle-test` | Endpoint di test per i webhook Paddle | infra |

### `license-generator-programma` — dettagli rilevanti per il single-use

Riceve un payload in forma di webhook Paddle (oggi popolato a mano per i test):
`event.data.customer.email`, `event.data.items[0]` (con `product_id`,
`quantity`, opzionale `subscription_type`, opzionale `expiry_days`).

1. Legge `product_catalog` per `code = product_id` → prende `validity_days`,
   `app_code`, `max_pdf_exports`.
2. Legge `apps.app_type` per quell'`app_code` (fallback `"programma"`).
3. Crea una riga in `purchases` (oggi `status: "test"`, in attesa del webhook Paddle reale).
4. Crea la riga in `licenses` con `license_key = TEST-<APP_CODE>-<8 hex>`,
   `seats = quantity`, `subscription_type` (whitelist:
   `"monthly" | "annual" | "single_use"`, altrimenti `null`),
   `pdf_exports_remaining = product.max_pdf_exports`.
   - Se `subscription_type !== "single_use"` → `expires_at` calcolata subito
     (`expiry_days` override o `validity_days` dal catalogo).
   - Se `subscription_type === "single_use"` → **`expires_at = null`** (le 48h
     partiranno dal primo accesso, non da qui — vedi nota in sezione 2, punto 9).
5. Recupera i `puk_codes` generati dal trigger DB sulla licenza appena creata.
6. Recupera `main_domain`/`pwa_domain` da `products` per costruire il link `/attivazione`.
7. Invia via Resend un'unica email con licenza + tutti i PUK + link di attivazione.

### `license-generator-corso` — differenze rispetto alla versione "programma"

- Richiede `course_id` nel payload (obbligatorio, family "corsi").
- `subscription_type` è **sempre `null`**: il single-use non esiste per questa
  family — coerente con `license-renew-programma` che blocca esplicitamente il
  rinnovo dei `single_use` (vedi sotto), un concetto pensato per i programmi,
  non per i corsi.
- Oltre alla scadenza della licenza, imposta separatamente una scadenza **per
  ogni PUK** (`puk_expiry_days`, default **3 mesi** dalla creazione) — i PUK dei
  corsi hanno quindi una finestra di utilizzo propria, indipendente da quella
  della licenza.

### `license-renew-programma` — guardia esplicita sul single-use

> *dal codice:* «Le licenze `single_use` non sono rinnovabili per definizione:
> durano 48h dal primo accesso o muoiono alla generazione del PDF. Un
> tentativo di rinnovo su questo tipo di licenza è quasi certamente un errore
> a monte (piano/prodotto sbagliato) e va segnalato esplicitamente.»

Se `existingLicense.subscription_type === "single_use"` la function risponde
`400` con `"Le licenze single_use non sono rinnovabili"` **senza** eseguire
alcun update. Per le altre licenze: `renewal_days` da payload, altrimenti `365`
per `annual`, altrimenti `30` (comportamento storico/`monthly`). La nuova
scadenza si somma a `expires_at` esistente se ancora futura (non si perde
tempo già pagato), altrimenti riparte da `now()`.

### `verify-license` e `test-license-generator` — da non confondere con l'app

- `verify-license` è generica e più datata (`updated_at` precedente a tutte le
  altre): non gestisce `subscription_type` né `pdf_exports_remaining`, quindi
  **non va confusa** con la logica app-side `verifyAndActivateLicense`
  descritta in sezione 2, che è più recente e più completa.
- `test-license-generator` sembra il predecessore diretto di
  `license-renew-programma` (stessa logica, senza guardia single-use): utile
  solo come riferimento storico.

### Chiusura della licenza single-use: **non è una Edge Function, è codice app-side**

Trovata leggendo il repo `ingnichettigb/002MnFAT`. Non vive tra le Edge
Function del progetto Supabase condiviso, ma direttamente nell'app, in
**`src/lib/license.functions.ts`**.

**`decrementPdfExports({ licenseId })`** — server function chiamata subito
dopo la generazione del PDF finale:

1. Legge `id, pdf_exports_remaining, is_active` per quella licenza.
2. Se `pdf_exports_remaining === null` → illimitata, no-op.
3. Se `pdf_exports_remaining <= 0` → già esaurita in precedenza (doppio click,
   retry di rete): no-op idempotente, ritorna comunque `exhausted: true`.
4. Altrimenti decrementa di 1; **se il nuovo valore è `<= 0`, nello stesso
   update imposta anche `is_active = false`** — un solo round-trip, non due.
5. L'update ha una **guardia ottimistica anti-race**:
   `.eq('id', licenseId).eq('pdf_exports_remaining', valoreLetto)` — se nel
   frattempo un'altra richiesta ha già decrementato, l'update non tocca
   nessuna riga invece di decrementare due volte.
6. È **fail-open**: qualsiasi eccezione tecnica ritorna `{ remaining: null,
   exhausted: false }` senza bloccare — il PDF è già stato scaricato prima di
   questa chiamata, quindi un errore qui non deve mai sembrare un blocco
   all'utente.

Chiamata dal client in **`src/routes/report.tsx`**, dentro `handleGenerate()`:
dopo `generateFatPdf(...)` e il download del file, legge `LICENSE_ID_KEY` da
localStorage e chiama `decrementExports({ data: { licenseId } })`. Se
`exhausted: true`, mostra un dialog bloccante (`showExhausted()`); il badge
con i crediti residui (`setPdfExportsBadge`) viene aggiornato in entrambi i casi.

> Conferma diretta dal codice per le licenze `single_use`: nascono con
> `pdf_exports_remaining = 1` (impostato da `license-generator-programma`,
> sezione 8), quindi **questo stesso meccanismo generico** — pensato per tutte
> le licenze con plafond, non solo per il single-use — le "brucia"
> automaticamente alla prima generazione, senza bisogno di una function dedicata.

Confermato anche il resto del ciclo di vita single-use già ipotizzato in
sezione 2: in `verifyAndActivateLicense` (stesso file), al primo claim
(`activated_at` ancora `null`), se `subscription_type === "single_use"` lo
stesso `UPDATE` guardato da `.is('activated_at', null)` valorizza **anche**
`expires_at` con la scadenza a 48h — esattamente come indicato dal commento
nel codice di `license-generator-programma` (sezione 8).

---

## 9. Riepilogo codici di errore
